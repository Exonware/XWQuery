...
def find_all_nodes_by_type(ast: ParseNode, node_type: str, max_depth: int = MAX_AST_DEPTH) -> list[ParseNode]:
    """
    ...
    Returns:
        List of all matching ParseNodes (may be empty)
...
def extract_node_value(ast: ParseNode) -> Any:
    ...
    Args:
        ast: ParseNode to extract value from
    Returns:
...
def traverse_depth_first(ast: ParseNode, max_depth: int = MAX_AST_DEPTH) -> Iterator[ParseNode]:
    """
    ...
    Args:
        ...
        max_depth: Maximum traversal depth (security limit)
    Yields:
        ParseNode in depth-first pre-order
...
def traverse_breadth_first(ast: ParseNode) -> Iterator[ParseNode]:
    ...
    Args:
        ast: Root AST node
    Yields:
        ParseNode in breadth-first order
    Example:
...
def find_parent(ast: ParseNode, target_node: ParseNode) -> ParseNode | None:
    ...
    Returns:
        Parent ParseNode, or None if target is root or not found
    Example:
...
def ast_to_dict(ast: ParseNode, max_depth: int = 10) -> dict[str, Any]:
    ...
    Example:
        ...
        >>> js = get_serializer(JsonSerializer)
        >>> out = js.dumps(ast_dict); print(out.decode('utf-8') if isinstance(out, bytes) else out)
    """
...
def extract_identifier_chain(ast: ParseNode) -> list[str]:
    ...
    Args:
        ast: ParseNode representing property reference
    Returns:
