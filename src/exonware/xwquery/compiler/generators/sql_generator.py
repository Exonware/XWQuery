...
class SQLGenerator(AStructuredQueryGenerator):
    ...
    def generate(self, actions: list[QueryAction], **options) -> str:
        ...
        action_type = getattr(first_action, 'type', getattr(first_action, 'operation', None))
        if action_type == 'SELECT':
            ...
            raise XWQueryValueError(
                f"Unsupported statement type: {action_type}\n"
    ...
    def _generate_select_statement(self, actions: list[QueryAction]) -> str:
        ...
        for action in actions:
            action_type = getattr(action, 'type', getattr(action, 'operation', None))
            if action_type == 'SELECT':
                ...
                from_table = action.params.get('source', action.params.get('table'))
            elif action_type in ('WHERE', 'FILTER'):
                where_expr = self._action_params_to_where_string(action.params)
            elif action_type == 'JOIN':
                joins.append(action)
            elif action_type == 'GROUP':
                group_by_cols = action.params.get('fields', action.params.get('columns', []))
                ...
                ascending = action.params.get('ascending', True)
                for field in fields:
                    direction = 'ASC' if ascending else 'DESC'
                    order_by_specs.append((field, direction))
            elif action_type == 'LIMIT':
                limit_value = action.params.get('count', action.params.get('limit'))
                offset_value = action.params.get('offset', 0)
            elif action_type == 'OFFSET':
        ...
        columns = select_action.params.get('fields', select_action.params.get('columns', ['*']))
        distinct = select_action.params.get('distinct', False)
        from_table = from_table or select_action.params.get('table')
