...
class XPathGenerator(APathQueryGenerator):
    ...
    def generate(self, actions: list[QueryAction], **options) -> str:
        ...
        for action in actions:
            # Use 'type' attribute instead of 'operation'
            action_type = getattr(action, 'type', getattr(action, 'operation', None))
            if action_type == 'SELECT':
                columns = action.params.get('fields', action.params.get('columns', ['*']))
            elif action_type == 'FROM':
                table = action.params.get('source', action.params.get('table'))
            elif action_type in ('WHERE', 'FILTER'):
                predicate = self._action_to_predicate(action)
                if predicate:
                    predicates.append(predicate)
            elif action_type in ('JOIN', 'GROUP', 'ORDER', 'LIMIT', 'OFFSET'):
