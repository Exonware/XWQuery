...
class XPathParser(APathQueryParser):
    ...
    def _location_path_to_actions(self, location_path: dict[str, Any]) -> list[QueryAction]:
        ...
        # Build actions - create SELECT first with source as table
        ...
        for step in filter_steps:
            ...
            for predicate in step['predicates']:
                if predicate['type'] == 'comparison':
                    ...
                    if op == '=':
                        where_conditions.append({left: right})
                    elif op == '>':
                        where_conditions.append({left: {'$gt': right}})
                    elif op == '<':
                        where_conditions.append({left: {'$lt': right}})
                    elif op == '>=':
                        where_conditions.append({left: {'$gte': right}})
                    elif op == '<=':
                        where_conditions.append({left: {'$lte': right}})
                    elif op == '!=':
                        where_conditions.append({left: {'$ne': right}})
        ...
        if where_conditions:
            if len(where_conditions) == 1:
                ...
                where_expr = {'$and': where_conditions}
        # Add select action with source as table (not separate FROM action)
        builder.select(['*'], table=source)
        # Add WHERE if we have conditions
        if where_expr:
            builder.where(where_expr)
        # Return as list (parser expects list[QueryAction])
