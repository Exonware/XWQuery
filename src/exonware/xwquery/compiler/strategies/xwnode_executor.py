...
class XWNodeQueryActionExecutor(AQueryActionExecutor):
    ...
    def execute_query(self, query: str, query_type: str, **kwargs) -> Any:
        ...
        if not self.validate_query(query, query_type):
            raise XWQueryValueError(f"Invalid {query_type} query: {query}")
        ...
        try:
            ...
            raise XWQueryValueError(f"Query execution failed: {e}")
    ...
    def _get_strategy(self, query_type: str) -> Any:
        ...
        if not strategy_class:
            raise XWQueryValueError(f"No strategy available for query type: {query_type}")
    ...
    def _get_strategy_class(self, query_type: str) -> Optional[type]:
        ...
        if not module_name:
            return None
        # Per DEV_GUIDELINES.md: No try/except for imports
        # Return None for unsupported query types (proper error raised in caller)
        return None

    def to_native(self) -> XWQSStrategy:
        """Convert to XWQSStrategy using actions."""
        return XWQSStrategy()

    def to_actions_tree(self, query: str) -> ANode:
        """Convert query to actions tree using XWQS."""
        script_strategy = XWQSStrategy()
        return script_strategy.parse_script(query).get_actions_tree()

    def from_actions_tree(self, actions_tree: ANode) -> str:
        """Convert actions tree to query using XWQS."""
        script_strategy = XWQSStrategy(actions_tree)
