...
def _get_env_var(key: str, default: Any, target_type: type):
    ...
    Args:
        ...
        default: The default value (should match target_type)
        target_type: The type to cast the value to (int, float, bool, str)
    Returns:
        The casted value
    Raises:
    ...
    try:
        ...
        raise XWQueryValueError(
            f"Invalid value for environment variable {key}: '{env_value}'. "
...
class XWQueryConfig:
    ...
    def from_env(cls) -> XWQueryConfig:
        ...
        for field in fields(cls):
            ...
            # Pass default as-is; _get_env_var will use it if env var doesn't exist
            kwargs[field.name] = _get_env_var(env_key, field.default, field.type)
        return cls(**kwargs)
