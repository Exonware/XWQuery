...
class SelectExecutor(AUniversalOperationExecutor):
    ...
    def _do_execute(self, action: QueryAction, context: ExecutionContext) -> ExecutionResult:
        ...
        if table_name:
            ...
            source_node = context.node.get(table_name) if hasattr(context.node, 'get') else context.node
    ...
    def _apply_order_by(self, data: List[Dict], order_by: str) -> List[Dict]:
        ...
        Root cause fixed: SELECT wasn't applying ORDER BY from parsed SQL.
        Solution: Parse order_by string and sort data accordingly.
        
        Args:
            data: List of dictionaries to sort
            order_by: ORDER BY clause (e.g., "age ASC", "price DESC")
