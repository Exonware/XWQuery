...
# ============================================================================
# Per DEV_GUIDELINES.md Line 128: NO try/except for imports
# xwnode is a required dependency - import directly
...
class DataValidator:
    """
    Wrapper for xwsystem validation.
    
    REUSE: Uses xwsystem validation (required dependency per DEV_GUIDELINES.md).
    """
    ...
    def validate_input(data: Any, operation: str = "query_operation") -> None:
        ...
        - Memory estimation
        """
        # Use xwsystem validation directly - it's a required dependency
        try:
    ...
    def validate_field_name(field: str, operation: str = "query") -> None:
        ...
        REUSE: xwsystem.validation.validate_path_input
        """
        if not field:
    ...
    def estimate_memory(data: Any) -> int:
        ...
        REUSE: xwsystem.validation.estimate_memory_usage
        """
        # Use xwsystem validation directly - it's a required dependency
        try:
            return estimate_memory_usage(data)
        except Exception:
            # If estimation fails, use sys.getsizeof as safe fallback
...
class SafeExtractor:
    ...
    REUSE: Uses xwnode traversal (required dependency per DEV_GUIDELINES.md).
    """
    ...
    def extract_items(node: Any, validate: bool = True) -> List[Any]:
        ...
        # REUSE: XWNode traversal - xwnode is a required dependency
        if isinstance(node, XWNode):
            try:
