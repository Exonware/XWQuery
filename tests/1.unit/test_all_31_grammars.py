...
GRAMMAR_DIR = PROJECT_ROOT / "src" / "exonware" / "xwquery" / "query" / "grammars"
...
def _run_grammar_test(name, grammar_text, test_queries):
    """Run a single grammar test and return success boolean."""
    print(f"\n{'='*60}")
...
@pytest.mark.parametrize("lang_name", sorted(LANGUAGE_TESTS.keys()))

def test_grammar(lang_name):
