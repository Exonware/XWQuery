...
def get_version_py_path() -> Path:
    ...
    m = re.search(r"\[tool\.setuptools\.dynamic\][^\n]*version\s*=\s*\{[^}]*file\s*=\s*[\"']([^\"']+)[\"']", text, re.DOTALL)
    if m:
        return (ROOT / m.group(1).strip()).resolve()
    m = re.search(r"version\s*=\s*\{\s*file\s*=\s*[\"']([^\"']+)[\"']", text, re.DOTALL)
    if m:
        return (ROOT / m.group(1).strip()).resolve()
    print("ERROR: [tool.hatch.version] path or [tool.setuptools.dynamic] version file not found in pyproject.toml", file=sys.stderr)
    sys.exit(2)
...
def get_canonical_date(version_py: Path) -> str | None:
    """Read __date__ from version.py (DD-MMM-YYYY). Returns None if not present.
    Supports both string literals and dynamic __date__ (e.g. from _today_release_date()).
    """
    if not version_py.exists():
        return None
    code = version_py.read_text(encoding="utf-8")
    ...
    if m:
        return m.group(1)
    # Dynamic __date__ (e.g. _today_release_date()): resolve at runtime
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location("_version_check", version_py)
        mod = importlib.util.module_from_spec(spec)
