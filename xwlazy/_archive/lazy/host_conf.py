...
from .lazy_contracts import get_preset_mode
        ...
        if name == "lazy":
            ...
            for pkg_name in package_names:
                if is_lazy_install_enabled(pkg_name):
                    from .lazy_core import LazyInstallConfig
                    mode_config = LazyInstallConfig.get_mode_config(pkg_name)
                    if mode_config:
                        # Return preset name if matches, otherwise return mode string
                        for preset_name, preset_config in get_preset_mode.__globals__.get("PRESET_MODES", {}).items():
