...
class LazyInstaller(
    ...
    def install_package(self, package_name: str, module_name: str = None) -> bool:
        ...
        try:
            with self._lock:
                ...
                # First check if it's a standard library module (should never install via pip)
                if check_name and self._is_stdlib_module(check_name):
                    _log("install", logger.debug, f"Module {check_name} is part of standard library, skipping installation")
                    # Mark as "installed" (it's built-in) to avoid future checks
                    self._installed_packages.add(package_name)
                    self._install_cache.mark_installed(package_name, "stdlib")
                    return True
    ...
    def _is_module_importable(self, module_name: str) -> bool:
        ...
        # Check if it's a standard library module (should never be installed via pip)
        ...
        try:
            ...
            # Check if it's a standard library module by origin
            if spec.origin is None:
                # Built-in module (like sys, builtins)
                return True
            
            # Check if origin is in standard library paths
            if spec.origin and 'site-packages' not in spec.origin:
                # Likely standard library (not in site-packages)
                stdlib_paths = [
                    sys.prefix,
                    sys.base_prefix,
                    sys.exec_prefix,
                ...
                for stdlib_path in stdlib_paths:
                    if stdlib_path and stdlib_path in spec.origin:
                        # Check if it's in lib/pythonX.X directory (stdlib) vs site-packages
                        if 'lib' in spec.origin and 'site-packages' not in spec.origin:
                            return True
            
            # Module exists and has a loader
            return spec.loader is not None
    ...
    def _is_stdlib_module(self, module_name: str) -> bool:
        ...
        # This is a heuristic - Python doesn't provide a definitive way to check
        # but we can check if the module spec origin is in stdlib paths
        try:
