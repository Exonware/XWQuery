#exonware/xwsyntax/src/exonware/xwsyntax/handlers/toml.py
"""
TOML Grammar Handler - Self-describing, NO HARDCODING!
...
class TOMLGrammarHandler(ASyntaxHandler):
    """TOML Grammar Handler - all metadata self-declared!"""
    # =========================================================================
    ...
    def mime_types(self) -> list[str]:
        return ["application/toml", "text/x-toml"]
    @property
    ...
    def _get_default_grammar(self) -> AGrammar:
        """Get default TOML input grammar."""
        from ..facade import XWSyntax
        engine = XWSyntax()
        return engine.load_grammar("toml")
    def _get_default_output_grammar(self) -> AGrammar:
        """Get default TOML output grammar."""
        from ..grammar_loader import get_grammar_loader
        loader = get_grammar_loader()
        grammar_paths = [
            "toml.out.grammar",
            "grammars/toml.out.grammar",
            "src/exonware/xwsyntax/grammars/toml.out.grammar",
        ]
