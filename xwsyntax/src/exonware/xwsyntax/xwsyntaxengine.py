#exonware/xwsyntax/src/exonware/xwsyntax/xwsyntaxengine.py
"""
Core syntax engine implementations.
This module provides multiple syntax engine implementations:
...
class XWSyntaxEngine(ASyntaxEngine):
    ...
    def __init__(
        self,
        grammar_dir: Optional[str | Path] = None,
        cache_size: int = 128,
    ):
        """
    ...
    def _resolve_grammar_dir(self, grammar_dir: Optional[str | Path]) -> Path:
    ...
    def _detect_grammar_file(self, name: str) -> Optional[Path]:
        ...
        """
        extensions = [
            '.in.grammar',  # Input grammar files (preferred for parsing)
            '.grammar',
            '.tmLanguage.json',
            '.tmLanguage',
            '.json',
    ...
    def list_grammars(self) -> list[str]:
        ...
        if not self._grammar_dir.exists():
            return []
        grammar_names = set()
        # Find all .in.grammar files first (preferred)
        ...
        for file in self._grammar_dir.glob("*.grammar"):
            name = file.stem
            # Skip .in.grammar and .out.grammar (already processed)
